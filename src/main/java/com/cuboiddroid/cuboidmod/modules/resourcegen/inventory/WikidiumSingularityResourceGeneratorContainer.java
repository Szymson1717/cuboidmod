...
public class WikidiumSingularityResourceGeneratorContainer extends SingularityResourceGeneratorContainerBase {
    ...
    public WikidiumSingularityResourceGeneratorContainer(int windowId, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModContainers.WIKIDIUM_SINGULARITY_RESOURCE_GENERATOR.get(), windowId, level, pos, playerInventory, player);
    }

    @Override
    public boolean stillValid(Player player) {
        return isWithinUsableDistance(ContainerLevelAccess.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerEntity, ModBlocks.WIKIDIUM_SINGULARITY_RESOURCE_GENERATOR.get());
    }
}