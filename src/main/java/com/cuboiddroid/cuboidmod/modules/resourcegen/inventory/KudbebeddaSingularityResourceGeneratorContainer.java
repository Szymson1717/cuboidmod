...
public class KudbebeddaSingularityResourceGeneratorContainer extends SingularityResourceGeneratorContainerBase {
    ...
    public KudbebeddaSingularityResourceGeneratorContainer(int windowId, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModContainers.KUDBEBEDDA_SINGULARITY_RESOURCE_GENERATOR.get(), windowId, level, pos, playerInventory, player);
    }

    @Override
    public boolean stillValid(Player player) {
        return isWithinUsableDistance(ContainerLevelAccess.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerEntity, ModBlocks.KUDBEBEDDA_SINGULARITY_RESOURCE_GENERATOR.get());
    }
}