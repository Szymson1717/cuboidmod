...
public class NotarfbadiumQuantumCollapserContainer extends QuantumCollapserContainerBase {
    ...
    public NotarfbadiumQuantumCollapserContainer(int windowId, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModContainers.NOTARFBADIUM_QUANTUM_COLLAPSER.get(), windowId, level, pos, playerInventory, player);
    }

    @Override
    public boolean stillValid(Player player) {
        return isWithinUsableDistance(ContainerLevelAccess.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerEntity, ModBlocks.NOTARFBADIUM_QUANTUM_COLLAPSER.get());
    }
}