...
public class NotsogudiumQuantumCollapserContainer extends QuantumCollapserContainerBase {
    ...
    public NotsogudiumQuantumCollapserContainer(int windowId, World level, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player) {
        super(ModContainers.NOTSOGUDIUM_QUANTUM_COLLAPSER.get(), windowId, level, pos, playerInventory, player);
    }

    @Override
    public boolean stillValid(PlayerEntity player) {
        return isWithinUsableDistance(IWorldPosCallable.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerEntity, ModBlocks.NOTSOGUDIUM_QUANTUM_COLLAPSER.get());
    }
}