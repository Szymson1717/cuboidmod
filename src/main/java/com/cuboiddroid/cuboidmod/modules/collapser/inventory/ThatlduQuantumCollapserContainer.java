...
public class ThatlduQuantumCollapserContainer extends QuantumCollapserContainerBase {
    ...
    public ThatlduQuantumCollapserContainer(int windowId, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModContainers.THATLDU_QUANTUM_COLLAPSER.get(), windowId, level, pos, playerInventory, player);
    }

    @Override
    public boolean stillValid(Player player) {
        return isWithinUsableDistance(ContainerLevelAccess.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerEntity, ModBlocks.THATLDU_QUANTUM_COLLAPSER.get());
    }
}